...
### 2026-08-28 -- G-TRK PASS (realized axis {0.681,1.647,3.183,6.402,12.889,19.846} m/s^2 for commanded {1,2.5,5,10,20,30}); a=30 via free carriage (grasped ejects instantly). Receipts g_trk_ladder.json + g_trk_axis.json. See DECISIONS 2026-08-28.

### 2026-08-28 -- Amendment 2: transport slip label v2.1 (external ruling). Slip = |residual world-x block-COM-vs-gripper displacement at settle-end| > 2mm OR drop/eject. max-3D peak + Y/Z residual demoted to recorded observables (never label). Transport is GPU-non-deterministic -> 3-seed + two-thirds band rules mandatory; T-EXT on confirmed labels only. a=1 row must be monotone in F + intact at 0.8-1.5N before the screen resumes. Receipt reports/vbd/slip_metric_issue.md. Damage/drop unchanged. See DECISIONS 2026-08-28.

### 2026-08-30 -- Amendment 3 (external Ruling 8, ralph/RULING-VG-20260830.md): VG certifies the pad contact-field channel only; W1 labels use the state-log channel, so W1 certification := finite AND vg3==0 (all 126 cells certified, classifier AUTHORIZED). vg2 (zero-record) demoted to diagnostic (unsatisfiable artifact). vg1 binds pad-channel/W2 claims only (18 violations all F2.0 damage; intact max 0.276mm; hard STOP if an intact pad-row ever violates). Classifier verdict = P-B CLOSURE; contraction descriptive. No label/band changes.
