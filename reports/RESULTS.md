...
**Pre-registered classifier verdict = P-B CLOSURE (AUTHORIZED, external Ruling 8/Amendment 3).** VG is scoped to the pad contact-field channel; W1 labels use the state-log channel, so W1 certification := finite AND vg3==0 -- both hold 476/476, so all 126 cells certify and the classifier is authorized. vg2 (zero record-dropout) is a demoted diagnostic (unsatisfiable artifact). First-match verdict on the confirmed labels is P-B CLOSURE (each material's intact band closes to empty and persists at higher a: E7/E15 at a=5, E25 at a=10); the contraction above is the descriptive narrative. Receipt: reports/logs/vbd/w1_classifier_verdict.json.
