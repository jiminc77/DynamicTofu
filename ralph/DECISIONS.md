...
Force sweep {0.3,0.5,0.8,1.2,2.0}N at soft_contact_mu 0.5 (+ extended 5N, 8N, mu 1.0/2.0, gentle 3s lift): ALL slip. The VBD tofu (E=25kPa, proper elastic) is grasped and LIFTS (peak 17-59mm, beyond MPM's zero) but slips/extrudes out during the 5s hold in every case (post-lift COM rise ~4.6mm = rest). Failure mode: low force -> friction slip; high force -> elastic bulge/extrusion around the pads (bbox y 0.04->0.059 at 8N); slower lift and higher friction do not help. CRITICAL: a proper elastic model at FIRMER E than MPM ALSO cannot fingertip-pinch-hold tofu -> two independent solvers agree, CONFIRMING the empty-band difficulty. Per directive: STOP + escalate with traces. Report reports/vbd/v2_report.md; traces reports/logs/vbd/v2_sweep.json; failure clip reports/media/v2_slip_2N.mp4. Levers for user: firmer E (material axis / min-E-that-holds sweep), pad geometry (shelf previously rejected), contact tuning, or accept the cross-solver-confirmed finding. MPM data stays frozen.
